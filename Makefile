...
LDFLAGS = -s -w \
	-X github.com/TessorNetwork/go-api/health.version=$(VERSION) \
	-X github.com/TessorNetwork/go-api/health.commit=$(COMMIT)
...
.PHONY: image
image:
	docker build -t theseus-local -f scripts/Dockerfile .
